 ...
 * Reference: the basic idea is from Trace.java created as part of DynamicAnalysis lab of System Re-engineering
 */
public aspect XYChartAspect {
	// This member variable is used to filter the methods being traced to jfreechart package i.e. org.jfree
	// To trace all function either assign it as empty or "*"
	// This can be further improved by having config.properties which could hold a parameter and control the behavior
	// of the packages to be considered or filtered externally. Intentionally this is kept simple for the assignment.
	...
	private void logHeader(){
		// Capturing TimeStamp, ClassName, FunctionName, Parameters of the function and StackDepth
	...
	pointcut traceMethods() : (execution(* *(..))&& !cflow(within(XYChartAspect)));

	// Increments a call or stack depth - invoked when a method is called for execution
	private static void incrementDepth() {
		stackDepth++;
	}
	
	// Decrements a call or stack depth - invoked when a method is returned after execution
	...
	// before() pointcut to consider execution trace before getting into a method call
	before(): traceMethods(){
		...
		if (isFilterPackage(className)) {
			...
			String trace =  LocalDateTime.now() + "\t" + className + "\t" + functionName
					 + "\t" + parameters + "\t" + line + "\t" + stackDepth;
